...
module Medreg
  ...
  Companies_XLSX      = File.expand_path(File.join(__FILE__, '../../../data/companies_latest.xlsx'))
  ...
  class Company
      ...
      glns.each { |gln|
        ...
        while nr_tries < max_retries  and not success
          ...
          if (@companies_created + @companies_updated) % 100 == 99
            Medreg.log "Start saving @app.companies.odba_store #{gln} after #{@companies_created} created #{@companies_updated} updated"
            @app.companies.odba_store
            Medreg.log "Finished @app.companies.odba_store" if DebugImport
          end
        end
      }
      r_loop.finished
    ensure
      Medreg.log "Start saving @app.companies.odba_store"
      @app.companies.odba_store
      Medreg.log "Finished @app.companies.odba_store"
    end
    def get_latest_file
      agent = Mechanize.new
    ...
    def store_company(data)
      ...
      if(company = @app.company_by_gln(data[:ean13]))
        ...
        pointer = company.pointer
        Medreg.log "  created #{company} #{company.pointer}"
        action = 'create'
      end
      ba_type = nil
      case  data[:ba_type]
      ...
      company.odba_store
      @app.companies.odba_store
      Medreg.log "store_company updated #{data[:ean13]} oid #{company.oid}  #{action} in database. pointer #{pointer.inspect} ba_type #{ba_type}. Have now #{@app.companies.size} companies."
